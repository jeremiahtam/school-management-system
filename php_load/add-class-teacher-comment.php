...
if(isset($_POST['session'])&&isset($_POST['reg_no'])&&isset($_POST['term'])&&isset($_POST['class_teacher_comment'])){
	...
	$form_teacher_comment = htmlentities($_POST['class_teacher_comment']);
	$date = date('Y-m-d');
	$time = date('H:i:s');
	...
	if($num_rows==0){
		...
		$sql = mysqli_query($conn,"INSERT INTO class_teacher_comment VALUES(default,'$studentId','$session','$term','$class_teacher_name','$form_teacher_comment','$date','$time')");
		if($sql){
			echo"<div class='text-success bg-success'>Successfully Inserted</div>";
		}
	}if($num_rows==1){
		//update attendance
		$sql = mysqli_query($conn,"UPDATE class_teacher_comment SET class_teacher_name='$class_teacher_name',comment='$form_teacher_comment',date='$date',time='$time' WHERE student_id='$studentId' AND term='$term' AND session='$session'");
		if($sql){
