...
switch(true){
	//only load this if its NOT EDIT but ADDITION of teachers
	case !isset($_POST['edit_id'])&&isset($_POST['fullname'])&&isset($_POST['username'])&&isset($_POST['password'])&&isset($_POST['email']):
	...
	$check_username_query = "SELECT * FROM all_teachers WHERE username='$form_username' AND removed='no'";	
	...
	if($check_username_num_rows>0){
		...
		$insert_sql = mysqli_query($conn,"INSERT INTO all_teachers VALUES (default,'$form_fullname','$form_username','$form_password','$form_email','no','$date','$time')");
	...
	//only load this if its EDIT teachers
	case isset($_POST['edit_id'])&&isset($_POST['fullname'])&&isset($_POST['username'])&&isset($_POST['password'])&&isset($_POST['email']):
	include("../classes/develop-php-library.php");
	include("../inc/session.inc.php");
	include("../inc/db.inc.php");
	...
	//only load this if its DELETE teachers
	...
	//load teachers if its not delete, edit or add teachers
	default:
